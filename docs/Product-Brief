...
PulseReq replaces the paper with a tokenized, expiring link. The clinician issues a LOINC-coded order; the patient books where and when and receives the link; the diagnostic centre receives a structured order it can resolve at intake without re-keying. Primary user: diagnostic centre intake staff, who carry the re-keying burden today. Secondary users: the ordering clinician and the patient, each with a distinct portal and a distinct set of allowed actions.

Goals (v1): eliminate lab re-keying at intake; make a requisition unlosable (recoverable by token or from a patient-facing list); model who is allowed to see a sensitive result and when. Non-goals (v1): EMR ingest (orders are entered by hand in this prototype, not received via webhook); LIS egress (PulseReq hands off to the lab; it never results a test); authentication (role is determined by URL, not login); an auditor or compliance-officer view of the audit ledger.
...
Initial build: the Results review inbox grouped items by whether the clinician had clicked "release." Error found: this produced a wrong system state — a result whose embargo window had already lapsed was visible to the patient regardless of clinician action, but the inbox still showed it as unresolved, contradicting what the patient could already see. The inbox was organized around an internal action (has staff clicked a button) rather than the outcome that actually matters (can the patient see this right now). Change made: regrouped the inbox around patient-facing visibility — "Awaiting decision" vs. "Released to patient" — so the queue reflects reality instead of internal process state. Result: the strongest product-thinking artifact in the build — a modeling error, not a code error, caught by asking what the other portal's user actually sees, fixed by changing the grouping key rather than patching the symptom.
