...
## Goal
Change both the doctor's "Extend" action and the patient's "Request more time" control to offer +7, +14, +21, and +28 day options, with 7 days as the default.
...
3. `src/components/extension-request-dialog.tsx`
   - Update the `useState<3 | 7 | 14>` for `days` to `useState<7 | 14 | 21 | 28>` and keep the default value of `7`.
...
- Open the Doctor Portal's "Issued log" and click "Extend" on an expired requisition to confirm the four pill options render correctly.
